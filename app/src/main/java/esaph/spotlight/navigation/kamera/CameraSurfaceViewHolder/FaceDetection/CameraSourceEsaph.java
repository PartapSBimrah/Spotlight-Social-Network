 ...
 * {@link CameraSourceEsaph.Builder#setRequestedFps(float)} to specify a frame rate that works well with
 ...
 * </ul>
 */
public class CameraSourceEsaph {
    ...
    public static class Builder {
        ...
        private CameraSourceEsaph mCameraSource = new CameraSourceEsaph();
    ...
    /**
     * Callback interface used to notify on auto focus start and pauseAndSeekToZero.
     * <p/>
    ...
    public CameraSourceEsaph start() throws IOException {
        synchronized (mCameraLock) {
            if (mCamera != null) {
                return this;
            }
     ...
     */
    @RequiresPermission(Manifest.permission.CAMERA)
    public CameraSourceEsaph start(SurfaceHolder surfaceHolder) throws IOException {
        synchronized (mCameraLock) {
            if (mCamera != null) {
                return this;
            }
    ...
    public void stop()
    {
        System.out.println("Stopped preview callback");
        synchronized (mCameraLock) {
            ...
            if (mProcessingThread != null) {
                try {
                    ...
                    // quickly after pauseAndSeekToZero).
    ...
    private Camera createCamera() {
        ...
        CLPreferences preferences = new CLPreferences(mContext);
        preferences.setCamWidth(mPreviewSize.getWidth());
        preferences.setCamHeight(mPreviewSize.getHeight());
    ...
    public void setPreviewCallBackListener(Camera.PreviewCallback previewCallBackListener)
    {
    ...
    private boolean mLockPreviewCallBack = false; //Need to be locked if taking a picture, to pauseAndSeekToZero flickering.
    private class CameraPreviewCallback implements Camera.PreviewCallback {
        ...
        public void onPreviewFrame(byte[] data, Camera camera)
        {
            ...
            {
                previewCallBackListener.onPreviewFrame(data, camera);
            }

            if(mLockPreviewCallBack) return;
            mFrameProcessor.setNextFrame(data, camera);
        }
    }

    public void setmLockPreviewCallBack(boolean mLockPreviewCallBack) {
        this.mLockPreviewCallBack = mLockPreviewCallBack;
    }
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        public void run() {
            ...
            while (true) {
                ...
                {
                    if(huntFace && graphicFaceTracker != null)
                    {
                        huntFace = false;
                        final byte[] mDataBuffered = data.array();
                        graphicFaceTracker.huntFace(new GraphicFaceTracker.FaceHunterCallback()
                        {
                            @Override
                            public void onHuntedFace(Face face)
                            {
                                cameraSourceDataArrayFrameListener.onFrameReceived(mDataBuffered, face);
                            }
                        });
                    }
    ...
    public interface CameraSourceDataArrayFrameListener
    {
        void onFrameReceived(byte[] data, Face face);
    }

    private CameraSourceDataArrayFrameListener cameraSourceDataArrayFrameListener;
    public void huntFace(CameraSourceDataArrayFrameListener cameraSourceDataArrayFrameListener)
    {
        this.cameraSourceDataArrayFrameListener = cameraSourceDataArrayFrameListener;
        this.huntFace = true;
    }

    private GraphicFaceTracker graphicFaceTracker;
    public void setGraphicFaceTracker(GraphicFaceTracker graphicFaceTracker) {
        this.graphicFaceTracker = graphicFaceTracker;
    }
